...
app.post('/api/register', (req, res) => {
    ...
    if (users[id]) return res.json({ success: false, msg: '이미 가입된 아이디입니다.' });
    users[id] = {
        id, password, name: id, statusMsg: '즐거운 대화, 혁찬톡!', bio: '반갑습니다!',
...
app.post('/api/login', (req, res) => {
    ...
    if (users[id] && users[id].password === password) {
        ...
        res.json({ success: false, msg: '아이디 또는 비번이 틀렸습니다.' });
    }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('join_private', (targetId) => {
        const room = [socket.userId, targetId].sort().join('-');
        socket.join(room);
        const history = privateMessages[room] || [];
        socket.emit('init_private', history);
    });
    ...
    socket.on('send_private', (data) => {
        ...
        io.to(room).emit('new_private', msg);
        // 대상자에게 새 메시지 알림 (뱃지 업데이트용)
        io.to(to).emit('private_notif', { from: socket.userId });
    });
